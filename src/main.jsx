...
function getResponsiveConfig() {
  ...
  if (isMobile) {
    return {
      ...
      cameraStartFov: 50,
      cameraEndFov: 25,
      textSize: 0.04, // Smaller for mobile stack
      textHeight: 0.008,
      textYOffset: 0.32, // Positioned slightly higher
    ...
    return {
      ...
      cameraStartFov: 50,
      cameraEndFov: 25,
      textSize: 0.06, // Smaller for stacked layout on tablet
      textHeight: 0.01,
      textYOffset: 0.36, // Adjusted for stacked layout
...
// Lights - Multiple lights to eliminate dark areas
...
drone.userData.flightDuration = 700; // Increased to slow down entire flight (launch + landing)
...
mtlLoader.setResourcePath('/src/Drone_Costum/Material/');
mtlLoader.setPath('/src/Drone_Costum/Material/');
mtlLoader.load('drone_costum.mtl', function (materials) {
  materials.preload();
  const objLoader = new OBJLoader();
  objLoader.setMaterials(materials);
  objLoader.setPath('/src/Drone_Costum/Material/');
...
loader.load('fonts/gentilis_bold.typeface.json', function (font) {
  ...
  if (responsiveConfig.isStacked) {
    ...
    const line1 = createLine('INNOVATION', responsiveConfig.textSize * 0.7);
    const line2 = createLine('2026', -responsiveConfig.textSize * 0.7);
  ...
  const boxGeometry = new THREE.BoxGeometry(0.5, 0.1, 0.05);
  textMesh = new THREE.Mesh(boxGeometry, textMaterial);
  textMesh.position.set(0, 2, 0);
  textMesh.scale.set(0, 0, 0);
  textMesh.renderOrder = 1000;
  textMesh.material.depthTest = false;
...
let render = function () {
  ...
  if (!drone.userData.isFlightComplete) {
    ...
    drone.userData.flightDuration = 700;
  ...
  if (revealProgress >= 0.90 && textMesh) {
    ...
    // Position text above the landing zone with floating effect
    let floatY = Math.sin(Date.now() * 0.003) * 0.02;
    let textY = currentEarthRadius + (responsiveConfig.textYOffset || 0.4) + floatY;
    textMesh.position.set(0, textY, 0);
    textMesh.lookAt(camera.position);

    // Debug logging
    if (textAnimationTime < 0.1) {
      console.log('Text animation started. Position:', textMesh.position, 'Scale:', currentEarthScale);
    }

    // Creative reveal with more visible scaling
    if (textAnimationTime < 2.0) {
      let t = Math.min(textAnimationTime / 2.0, 1);
      // Scale from 0 to 1 with ease out
      let s = t * t * (3.0 - 2.0 * t); // Smooth step function
      ...
      // Classy slow pulse after reveal
      let pulseScale = 1 + Math.sin(Date.now() * 0.0005) * 0.02;
      textMesh.scale.multiplyScalar(pulseScale);
    }
  }
    ...
    if (textMesh) {
      ...
      loader.load('fonts/gentilis_bold.typeface.json', function (font) {
        if (responsiveConfig.isStacked) {
          ...
          textMesh = textGroup;
          textMesh.isGroup = true; // Tag for disposal
        } else {
          // Single line for others
          const textGeometry = new TextGeometry('INNOVATION 2026', {
            font: font,
            size: responsiveConfig.textSize,
            height: responsiveConfig.textHeight,
        ...
        textMesh.position.set(0, 0.4, 0); // Placeholder, will be updated in render loop
        scene.add(textMesh);
      });
    }
  }
